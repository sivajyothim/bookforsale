...
class Crud extends CI_Model{
    ...
    public function singleTableData($params)
    {
        ...
        $return_record_type =(isset($params['record_return']))?$params['record_return']:'';
        ...
        if($dbError['code']==0)
        {
           ...
           if($return_record_type == 'single')
           {
               $response[$response_param]=($count > 0)?$sql->row():array();
           }
        }
        else
        {
    ...
    public function fetchSingleJoinsData($params)
    {
        $response=[];
        $debug = $params['debug'];
        $table =strtolower(trim($params['table']));
       ...
       // print_r($where);exit;
        ...
        if(is_array($joins) && count($joins) > 0)
        {
          //  print_r($joins);
                ...
                //print_r($joins_data);
                $j_tbl =trim(strtolower($joins['table']));
                $j_main_column =trim(strtolower($joins['original_column']));
                $j_ref_column =trim(strtolower($joins['ref_column']));
                $j_join_condition =  trim(strtoupper($joins['condition']));
