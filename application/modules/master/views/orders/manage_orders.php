                    ...
                    <div class="ibox-body">
                        ...
                        <div class="table-responsive row">
                            <table class="table table-bordered table-hover" id="datatable">
                                ...
                                <tbody>
                                <?php
                                $req = json_decode($order_details);
                                if($req->code == 200){
                                    $sno=1;
                                    foreach($req->order_result as $order_res){
                                        $disp_status='';
                                        $order_link=ADMIN_LINK.'Orders/orderdetails/'.$order_res->order_num.'/'.$order_res->order_id;
                                ...
                                <td><?php echo date('d-M-Y, h:i A',strtotime($order_res->created_on)); ?></td>
                                <td><?php echo $order_res->payment_mode; ?></td>
                                <td>
                                    <?php if($order_res->order_status==0){?>
                                    <a onclick="return confirm('Confirm to make this order as delivered ??')" href="<?php echo ADMIN_LINK; ?>Orders/maketodeliver/<?php echo $order_res->order_id; ?>" class="btn btn-sm btn-warning">Mark as Delivered</a>
                                <?php } ?>
                                    <?php echo ($order_res->order_status ==6)?date('d-M-Y, h:i a',strtotime($order_res->delivered_on)):''; ?>
                                        
                                </td>
                                <td><?php echo ($order_res->payment_status==1)?'Received':'Pending';?></td>
