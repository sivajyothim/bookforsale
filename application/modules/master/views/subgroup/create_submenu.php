...
<body class="fixed-navbar">
                            ...
                            <form class="form-info" action="" id="create_form" enctype="multipart/form-data" name="create_form">
                                <div class="ibox-body">
                                    ...
                                    <div class="col-sm-6 form-group mb-4">
                                            <label>Menu <sup class="error">*</sup></label>
                                        ...
                                        <div class="col-sm-6 form-group mb-4">
                                            <label>Sub Group Title <sup class="error">*</sup></label>
                                            <input class="form-control" type="text" placeholder="ECE/CSE/MPC/Bi.pc etc.." name="title" id="title" maxlength="60" autocomplete="off"/>
                                            <div class="error" id="title_error"></div>
                                        </div>

                                          <div class="col-sm-6 form-group mb-4">
                                            <label>Course Year<sup class="error">*</sup></label>
                                            <input class="form-control" type="text" placeholder="years" name="years" id="years" maxlength="1" autocomplete="off"/>
                                            <div class="error" id="years_error"></div>
                                        </div>
                                        

                                        <div class="col-sm-6 form-group mb-4">
                                            <label>Upload  Icon </label>
                                            <input class="form-control" accept="image/*" type="file" title="Upload file" name="upload_file" id="upload_file"/>
                                            <div class="error" id="upload_file_error"></div>
                                        </div>
                                    </div>
    ...
    $('#create_form').on('submit', function (e) {
        e.preventDefault();
        str = true;
        $('#title_error,#menu_error,#years_error').html('');
        var title= $('#title').val();
        var menu= $('#menu').val();
        var year= $('#years').val();
        if (title == 0 || title == '') {  $('#title_error').html('Please enter subgroup name');  str = false; }
        if (menu == 0 || menu == '') {  $('#menu_error').html('Please choose menu');  str = false; }
        if (year == '') {  $('#year_error').html('Please enter course years');  str = false; }
        ...
        if (str == true)
        {
         ...
         $('.disp_msg').html('Please wait....').css({'color': 'blue'});
            $.ajax({
                ...
                url: "<?php echo ADMIN_LINK; ?>Settings/insertsubgroup",
                contentType: false,
                cache: false,
                processData: false,
                success: function (s) {
                    ...
                    switch (s.code)
                    {
                        case 200:
                            ...
                            $('.disp_msg').html(s.description).css({'color': 'green', 'font-size':
                                        '15px'});
                            setTimeout(function () {
                                window.location = "<?php echo ADMIN_LINK . 'Settings/subgroup'; ?>";
                            }, 2000);
