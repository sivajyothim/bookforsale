...
<body class="fixed-navbar">
                ...
                <div class="ibox">
                    <div class="ibox-body">
                        ...
                        <div class="row pull-right">
                            <button  type="button" onclick="activateData(1,'user');" class="btn btn-success statusActivate">Acive</button>&nbsp;&nbsp;
                  <button type="button" onclick="activateData(0,'user');" class="btn btn-danger statusActivate">In-active</button>
                        </div>
                        ...
                        <div class="flexbox mb-4">
                            <div class="flexbox">
                                ...
                                <select class="selectpicker show-tick form-control" id="type-filter" title="Please select" data-style="btn-solid" data-width="150px">
                                    ...
                                    <option value="Order Placed">Active</option>
                                    <option value="Delivered">In-active</option>
                                 </select>
                            </div>
                        ...
                        <div class="table-responsive row">
                            <table class="table table-bordered table-hover" id="datatable">
                                ...
                                <tbody>
                                <?php
                                $req = json_decode($user_details);
                                if($req->code == 200){
                                    $sno=1;
                                    foreach($req->user_result as $user_res){
                                        $disp_status='';
                                        $orders_link=ADMIN_LINK.'User/userorders/'.url_title($user_res->user_name).'/'.$user_res->user_authtoken;
                                        
                                ?>
                                <tr>
                                <td><?php echo $sno; ?>
                                    <input type="checkbox" name="multiple[]" class="checkthis" value="<?php echo $user_res->user_id; ?>" />
                                </td>
                                <td><?php echo $user_res->user_name; ?></td>
                                <td><?php echo $user_res->email_id; ?></td>
                                <td>Rs.<?php echo $user_res->mobile_number; ?></td>
                                <td><?php echo $user_res->city; ?></td>
                                <td><?php echo $user_res->pincode; ?></td>
                                <td><?php echo date('d-M-Y, h:i A',strtotime($user_res->created_on)); ?></td>
                               
                                
                                <td><?php echo ($user_res->profile_status==1)?'Active':'Blocked';?></td>
                                <td>
                                    <a href="javascript:void(0)" onclick="viewUserDetails('<?php echo $user_res->user_authtoken; ?>')" class="btn btn-sm btn-primary">User Details</a>
                                    <a href="<?php echo $orders_link; ?>" class="btn btn-sm btn-success">Manage orders</a>
                                 <a onclick="return confirm('Confirm to Delete User ??')" href="<?php echo ADMIN_LINK; ?>User/deleteuser/<?php echo $user_res->user_authtoken; ?>" class="btn btn-sm btn-danger">Delete</a></td></td>
                                </tr>
                                <?php $sno++; } } else {?>
								<tr>
                                    <td colspan="9">
    ...
    <!-- END QUICK SIDEBAR-->
    <!--user details popup module section code start -->
...
<div class="modal-dialog">
  <div class="modal-content">
    ...
    <!-- User details popup module section code end -->
    <!-- CORE PLUGINS-->
    ...
    <script>
        ...
        function viewUserDetails(s)
        {
                ...
                if (r.code == 200)
                {
                    ...
                    $('#username').html(userres.username);
                    $('#useremail').html(userres.email);
                    $('#usermobile').html(userres.mobile);
                    $('#pincode').html(userres.pincode);
                    $('#address').html(userres.address);
                    $('#area').html(userres.area);
                    $('#country').html(userres.country);
                    $('#city').html(userres.city);
                    $('#state').html(userres.state);
                    $('#country').html(userres.country);
                    $('#landmark').html(userres.landmark);
                    $('#useremail').html(userres.email);
