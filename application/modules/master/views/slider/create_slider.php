...
<body class="fixed-navbar">
                                ...
                                <div class="ibox-body">
                                    <div class="row">
                                        <div class="col-sm-6 form-group mb-4">
                                            <label>Slider Title <sup class="error">*</sup></label>
                                            <input class="form-control" type="text" placeholder="Title" name="title" id="title" maxlength="60" autocomplete="off"/>
                                            <div class="error" id="title_error"></div>
                                        </div>
                                        

                                        <div class="col-sm-6 form-group mb-4">
                                            <label>Slider Image ( ** X **) </label>
                                        ...
                                        <div class="col-sm-6 form-group mb-4">
                                            <label>Slider Title <sup class="error">*</sup></label>
                                            <input class="form-control" type="text" placeholder="Slider Link" name="sliderlink" id="sliderlink" maxlength="300" autocomplete="off"/>
                                            <div class="error" id="title_error"></div>
                                        </div>
                                    </div>

                                <div class="ibox-footer hideclass">
                                    <button class="btn btn-info mr-2" type="submit">Upload</button>
    ...
    $('#create_form').on('submit', function (e) {
        e.preventDefault();
        str = true;
        $('#title_error,#upload_file_error').html('');
        var title= $('#title').val();
        var upload_file = $('#upload_file').val();
        if (title == 0 || title == '') {  $('#title_error').html('Please enter slider title');  str = false; }
        if (upload_file == '') {  $('#upload_file_error').html('Please upload slider');  str = false; }
        ...
        if (str == true)
        {
         ...
         $('.disp_msg').html('Please wait....').css({'color': 'blue'});
            $.ajax({
                ...
                url: "<?php echo ADMIN_LINK; ?>Settings/insertSlider",
                contentType: false,
                cache: false,
                processData: false,
                success: function (s) {
                    ...
                    switch (s.code)
                    {
                        case 200:
                            ...
                            $('.disp_msg').html(s.description).css({'color': 'green', 'font-size':
                                        '15px'});
                            setTimeout(function () {
                                window.location = "<?php echo ADMIN_LINK . 'Settings/sliders'; ?>";
                            }, 2000);
